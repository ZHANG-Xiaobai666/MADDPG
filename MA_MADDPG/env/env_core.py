...
class EnvCore:
    ...
    def __init__(self, args, arr_pro):
        ...
        self.num_channel = 1                            # Number of channels
        ...
        self.nodes_reward = np.zeros(self.num_agent)
        self.vl = np.zeros(self.num_agent)
        self.vl_ = np.zeros(self.num_agent)
    ...
    def reset(self):
        ...
        self.obs = [[np.zeros(self.obs_obs_dim) for _ in range(self.obs_his_len)] for _ in range(self.num_agent)]
        self.nodes_reward = [0 for _ in range(self.num_agent)]
    ...
    def step(self, actions, time):  # actions NX1
        ...
        for agent in range(self.num_agent):
            if agent in pri_idx:
                rewards_ind[agent] = 1 if actions[agent] == 1 else -1/(1-Dl[agent])
                if nodes_feedback[agent] == 0:
                    rewards_global = np.ones(self.num_agent)
            else:
                rewards_ind[agent] = -1 if actions[agent] == 1 else 1
                if nodes_feedback[agent] == 0:
        ...
        actions_vector = []
        for agent in range(self.num_agent):
            action = np.array([1, 0]) if actions[agent] == 0 else np.array([0, 1])
            actions_vector.append(action)
            node_feedback = np.zeros(4)
            node_feedback[nodes_feedback[agent]] = 1
            d_l1 = np.array([self.vl[agent]/(self.vl[agent]+self.vl_[agent]+1e-20)])
            d_l1_ = 1 - d_l1
            new_obs = np.concatenate([action, node_feedback, d_l1, d_l1_])
            self.obs[agent] = self.obs[agent][1:] + [new_obs]
        ...
        return self.obs, actions_vector, self.alpha*rewards_global + (1-self.alpha)*rewards_ind
