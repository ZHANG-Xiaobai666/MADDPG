    ...
    def run(self):
        ...
        for episode in range(self.episodes):
            ...
            for step in range(self.episode_length):
                ...
                a = copy.copy(self.env.get_sum_success())

                throughput_his.append(a)

                if (step + 1) % 200 == 0:
                    Throughput_Ret = {'Throughput_Ret': throughput_his}
                    f_name = 'Throughput_Ret_' + '_N' + str(self.num_agent) + 'K' + str(self.num_channel) + '.mat'
                    savemat(f_name, Throughput_Ret)
                    print(f"Iteration: {step} / {self.episode_length}")
    ...
    def collect(self, obs):
        ...
        for ch in range(self.num_channel):
            ...
            for agent in range(self.num_agent):
                action = self.agents[agent].select_action(obs[ch][agent], deterministic=True)
